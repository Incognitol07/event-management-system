...
export function ResourceSelector({ 
  ...
  const isResourceAllocated = (resourceId: number) => {
    return existingAllocations.some(allocation => allocation.resourceId === resourceId);
  };

  const getResourceAllocation = (resourceId: number) => {
    return existingAllocations.find(allocation => allocation.resourceId === resourceId);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-gray-100 pb-6">
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-2xl font-light text-gray-900">Resource Management</h3>
            <p className="text-gray-600 mt-1">Allocate resources needed for this event</p>
          </div>
          <button 
      ...
      {existingAllocations.length > 0 && (
        <div className="space-y-4">
          <h4 className="text-lg font-medium text-gray-900">Allocated Resources</h4>
          <div className="space-y-3">
            {existingAllocations.map(allocation => (
              <div key={allocation.id} className="border border-gray-100 p-6 hover:border-gray-200 transition-colors">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h5 className="font-medium text-gray-900">{allocation.resource.name}</h5>
                      <span className={`text-xs font-medium px-2 py-1 rounded-full ${getCategoryColor(allocation.resource.category)}`}>
                        {categoryDisplayNames[allocation.resource.category]}
                      </span>
                    </div>
                    <div className="text-sm text-gray-600">
                      Quantity: {allocation.quantityNeeded} of {allocation.resource.totalCount}
                    </div>
                    {allocation.notes && (
      ...
      {resources.length > 0 ? (
        <div className="space-y-4">
          <h4 className="text-lg font-medium text-gray-900">Available Resources</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {resources.map(resource => {
              ...
              return (
                <div key={resource.id} className={`border border-gray-100 p-6 transition-all duration-200 hover:border-gray-200 ${isAllocated ? 'opacity-60' : ''}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">Total: {resource.totalCount}</span>
                        <span className={resource.availableCount > 0 ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    {isAllocated ? (
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-medium px-3 py-1 border rounded-full ${getStatusColor(allocation.status)}`}>
                          Allocated ({allocation.quantityNeeded}) - {allocation.status}
                        </span>
                      </div>
                    ) : (
                      <button
        ...
        {selectedResource && (
          <div className="max-w-md mx-auto">
            ...
            <div className="bg-gray-50 p-6 mb-8">
              <h4 className="font-medium text-gray-900 mb-2">{selectedResource.name}</h4>
              <p className="text-sm text-gray-600 mb-3">{selectedResource.description}</p>
              <div className="text-sm">
                <span className="text-gray-500">Available: </span>
                <span className="font-medium">{selectedResource.availableCount} of {selectedResource.totalCount}</span>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  type="number"
                  value={quantity}
                  onChange={(e) => setQuantity(Math.max(1, Math.min(parseInt(e.target.value) || 1, selectedResource.availableCount)))}
                  min="1"
              ...
              <div className="flex space-x-4 pt-4">
                ...
                <button 
                  ...
                  disabled={isLoading || quantity > selectedResource.availableCount}
